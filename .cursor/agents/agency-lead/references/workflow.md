...
1. Read current status: check build logs or `.crush/agency/status.json` if present.
...
3. Merge outputs into a gap report.
4. When a **root cause hypothesis** exists, delegate `root-cause-healer` to verify, fix, audit imports, and review AI-surface hardening. Otherwise trigger `patch-builder` for structural gaps only.

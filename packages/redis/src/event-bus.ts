...
export interface SystemEventPayload {
  id: string;
  type: "db_drift" | "cache_invalidation" | "rate_limit_spike" | "system_error" | "telemetry_anomaly";
  source: string;
      ...
      switch (event.type) {
        ...
        case "cache_invalidation":
          if (typeof event.details?.prefix === "string") {
            ...
            console.log(`[Mini-SWE-Agent] Self-healing: Cleared stale prefix tags for '${event.details.prefix}' (${count} keys).`);
          }
          break;
        case "rate_limit_spike":
          console.log("[Mini-SWE-Agent] Self-healing: Applied load-adaptive throttling (adjusted window).");
          break;
        case "system_error":
